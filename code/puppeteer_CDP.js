...
async function mainLoop() {
	...
	await page.goto('https://archeage.ru/promo/aa81/index.html', {waitUntil : 'networkidle0' }).then(() => {
		console.log("Ok");
	}).catch(function(err) {
		console.error(err);
	});
	...
	for (var i in current_url_cookies) {
		if (current_url_cookies[i].name == "PHPSESSID") {
	...
	process.stdout.write("Asking ArchAge API is cookie valid... ");
